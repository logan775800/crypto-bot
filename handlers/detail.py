...
# ---------- 蜡烛图 + 综合研判（消息 2） ----------
# 顺序刻意是 **4h → 日线 → 周线**（2026-08-25 他要求改的）。
# 相册里第一张会被 Telegram 放大显示，其余缩成小图——所以第一张要放
# **最常看、最需要看清细节**的那个周期。他盯的是入场时机，那就是 4h；
# 周线是背景板，缩着看方向就够了。
# 注意：研判文案仍固定按**日线**算（MA3/13/23 的阈值是按日线定的语义），
# 图的排序和算研判用哪个周期是两件事，别一起改。
_TF_LABELS = [("4h", "4小时"), ("1d", "日线"), ("1w", "周线")]
...
async def build_multi_charts(symbol, flow=None):
    ...
    **研判固定用日线算**：均线口径 MA3/13/23 是按日线定的语义，
    ...
    caption = (f"{tfs}（第一张最大：先看 4h 找入场，再往后翻看大方向）｜K线来源：{src}\n"
               f"{text}\n⚠️ 仅供参考，不构成投资建议")
