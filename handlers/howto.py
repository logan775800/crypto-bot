...
def pinned_text():
    ...
    **最后那段是关键**：他抱怨的是"更新后新命令在哪不清楚"。
    ...
    return (
        ...
        "/pos BTC 持仓结构 —— 持仓量/爆仓量/大户仓位/散户账户数，还跟上一段行情对比\n"
        "　　`/pos BTR 13` 指定窗口；卡片上有 6h/13h/24h/48h 按钮\n"
        ...
        "🔮 /alpha 币安上币候选池 —— Alpha 里还没上现货的，按市值排；开告警后新进池就推\n"
        "　　⚠️ 不是预知上币，基础概率就 16%，作用是把几百个缩到十几个\n"
        "🩸 /liqflip 爆仓一边倒 —— 5分钟内一边被扫≥80%，摸顶抄底（判据是回测出来的）\n"
        "　　抄底看1小时(68%)、摸顶看4小时(34%)，两个尺度不一样，卡片上有写\n"
